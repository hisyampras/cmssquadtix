  ...
  <div class="max-w-3xl mx-auto p-4 md:p-8 pb-12 md:pb-16 space-y-6">
    ...
    <div class="rounded-3xl border border-slate-200/70 bg-white/80 backdrop-blur shadow-sm overflow-hidden
      ...
      <div class="p-5 md:p-6 border-b border-slate-200/70 dark:border-slate-800/70">
        ...
        <div class="text-xs text-slate-500 mt-1 dark:text-slate-400">Isi kode tiket dan data owner (opsional).</div>
      </div>

      <form method="POST" action="{{ route('events.tickets.store', $event) }}" class="p-5 md:p-6 space-y-4">
        @csrf
        ...
        <div>
          ...
          <input name="code"
                 ...
                 placeholder="Contoh: TCKT-000123"
                 required>

          @error('code')
            <p class="mt-2 text-xs font-semibold text-rose-700 dark:text-rose-200">
              {{ $message }}
            </p>
          @enderror

          <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">
            Disarankan unik. Bisa hasil generate dari sistem / import.
          </p>
        </div>
